#!/usr/bin/env python3
"""prototype/step1b_camoufox_click_capture.py — 阶段 3 Step 1 验证。
...
用法（需本机已起 camoufox-turnstile 服务，且能连真实 NVIDIA 会话）：
  ...
  python prototype/step1b_camoufox_click_capture.py
"""
...
# camoufox humanize 控制（浏览器层，影响 page.mouse.* 的鼠标移动速度）：
#   "true"  → humanize=True（默认 max 1.5s/次移动，最像真人但最慢）
#   "false" → humanize=False（不真人化，最快，但 checkbox 点击可能被 hCaptcha 拦）
#   数字    → humanize=<数字>（max 秒数，如 0.3 = 最多 0.3s/次，兼顾真人化与速度）
# checkbox 点击需要 isTrusted=true 绕过 hCaptcha，建议 true 或小数字（0.3-0.5）。
# 导航/填表用 locator.click()（DOM 点击）不受 humanize 影响，不会拖慢。
...
def _annotate(png_b64: str, dtype: str, answer, out_path: Path) -> Path | None:
    ...
    try:
        ...
        if dtype == "grid":
            ...
            # drag answer: [[sx,sy,ex,ey], ...] 画起点圆 + 终点圆 + 连线
            r = max(12, w // 50)
            for i, seg in enumerate(answer or []):
                try:
                    sx, sy, ex, ey = int(seg[0]), int(seg[1]), int(seg[2]), int(seg[3])
                except (ValueError, TypeError, IndexError):
                    continue
                # 起点绿圈、终点红圈、连线
                draw.ellipse([sx - r, sy - r, sx + r, sy + r], outline=(0, 200, 0),
                             width=max(3, w // 200))
                draw.ellipse([ex - r, ey - r, ex + r, ey + r], outline=(255, 0, 0),
                             width=max(3, w // 200))
                draw.line([sx, sy, ex, ey], fill=(255, 0, 0), width=max(2, w // 250))
                draw.text((sx + r + 4, sy - r), "S%d" % (i + 1),
                          fill=(0, 200, 0), font=font)
                draw.text((ex + r + 4, ey - r), "E%d" % (i + 1),
                          fill=(255, 0, 0), font=font)
        else:
            return None
...
async def _navigate_to_password_filled(page) -> bool:
    ...
    print("  [step1b] goto build.nvidia.com ...")
    ...
    try:
        ...
        pass
    print("  [step1b] password filled, hCaptcha widget ready")
    return True


async def main() -> int:
    print("=" * 60)
    print("  prototype/step1b_camoufox_click_capture.py — 阶段 3 Step 1")
    print(f"  classify_url = {CLASSIFY_URL}")
    print("  验证：自动点 checkbox + 取图 + 判图（不点击提交）")
    print("=" * 60)

    if not NV_EMAIL or not NV_PASSWORD:
        print("\n[step1b] NV_EMAIL / NV_PASSWORD 未设置")
        return 1

    # humanize=False：camoufox(humanize=True) 浏览器内核已真人化，不再加贝塞尔轨迹，
    # 直接 page.mouse.click 更快（camoufox 文档建议关自定义贝塞尔）。
    solver = ClassifySolver(api_url=CLASSIFY_URL, timeout_seconds=60, humanize=False)

    # camoufox（firefox 内核 + humanize）：浏览器内核层模拟真人鼠标事件
    # (isTrusted=true)，绕过 hCaptcha 对 page.mouse.click 的自动化检测。普通
    # chromium + 贝塞尔轨迹仍被 hCaptcha 拒（"请再试一次" + 不加载 challenge.js）。
    # HUMANIZE 环境变量控制：true/false/数字（max 秒）。导航填表用 locator.click
    # 不受影响，只有 checkbox 点击的 page.mouse.move 会真人化。
    print(f"  [step1b] camoufox humanize = {HUMANIZE}")
    async with AsyncCamoufox(headless=False, humanize=HUMANIZE) as browser:
        ...
        try:
            ...
            print("\n  [step1b] === Step 1: 自动点 checkbox ===")
            t0 = time.monotonic()
            clicked = await solver._click_checkbox(page)
            print(f"  [step1b] _click_checkbox = {clicked} "
                  f"({(time.monotonic() - t0):.1f}s)")
            if not clicked:
                print("\n[step1b] ❌ 自动点 checkbox 失败（最大风险点）")
                await _hold(HOLD_SECONDS, "checkbox 点击失败，可 F12 抓 checkbox iframe")
                return 3
            print("  [step1b] ✅ 自动点 checkbox 成功，挑战框已弹出")
            ...
            if challenge is None:
                ...
                return 4
            print(f"  [step1b] type={challenge['captcha_type']} "
                  f"js={challenge['challenge_js_type']!r} "
                  f"q={challenge['question']!r} "
                  f"size={challenge['grid_w']}x{challenge['grid_h']}")

            # bbox/unknown 不判图（classify 不支持 bbox；unknown 是未识别）
            if challenge["captcha_type"] in ("bbox", "unknown"):
                print(f"\n[step1b] ⏭️  type={challenge['captcha_type']} 不判图")
                await _hold(HOLD_SECONDS, "非 grid/point/drag 挑战，人眼对照挑战框")
                return 0
            ...
            print(f"  [step1b] answer = {answer} ({(time.monotonic() - t0):.1f}s)")
            if answer is None:
                print("\n[step1b] ❌ 判图失败")
